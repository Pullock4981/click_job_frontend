...
const CountryPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setSubmitting(true);
            if (editingId) {
                await api.put(`${API_ENDPOINTS.COUNTRIES}/${editingId}`, formData);
            } else {
                await api.post(API_ENDPOINTS.COUNTRIES, formData);
            }
            setFormData({ name: '' });
            setEditingId(null);
            fetchCountries();
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this country?')) {
            try {
                await api.delete(`${API_ENDPOINTS.COUNTRIES}/${id}`);
                fetchCountries();
        ...
        <AdminLayout>
            <div className="p-4 md:p-6 bg-[#F4F6F9] min-h-screen">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-xl font-normal text-gray-800">Country</h1>
                    <div className="text-xs text-blue-500">Home / All Country</div>
                </div>

                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    <div className="lg:col-span-8">
                        <div className="bg-white rounded shadow-md overflow-hidden">
                            <div className="bg-[#28a745] text-white p-3 font-bold text-sm">All Country</div>
                            <div className="p-4">
                                                    ...
                                                    <tr key={c._id} className="hover:bg-gray-50 border-b">
                                                        ...
                                                        <td className="p-3 border text-center">
                                                            <div className="flex justify-center gap-2">
                                                                ...
                                                                <button onClick={() => handleDelete(c._id)} className="bg-[#dc3545] text-white p-1.5 rounded hover:bg-red-600"><FaTrash size={12} /></button>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                ))
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-white rounded shadow-md overflow-hidden">
                            <div className="bg-[#007bff] text-white p-3 font-bold text-sm">New country</div>
