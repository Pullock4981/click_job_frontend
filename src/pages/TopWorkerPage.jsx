...
const TopWorkerPage = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [workerRes, headlineRes] = await Promise.all([
                api.get(`${API_ENDPOINTS.TOP_REPORTS}/top-workers`),
                api.get(`${API_ENDPOINTS.HEADLINES}?type=top-worker`)
            ]);
    ...
    const handleHeadlineSubmit = async (e) => {
        ...
        try {
            setSubmitting(true);
            await api.post(API_ENDPOINTS.HEADLINES, { ...formData, type: 'top-worker' });
        ...
        <AdminLayout>
            <div className="p-4 md:p-6 bg-[#F4F6F9] min-h-screen">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-xl font-normal text-gray-800">Top 50 Job Worker</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200 text-left text-[11px] font-bold text-gray-800 uppercase">
                                    ...
                                    <th className="p-4 border">Work</th>
                                </tr>
                            </thead>
                            <tbody className="text-[11px] text-gray-600">
                                {loading ? (
                                    ...
                                    workers.map((w, idx) => (
                                        <tr key={w._id} className="hover:bg-gray-50 border-b font-medium">
                                            <td className="p-4 border">{idx + 1}</td>
                                            <td className="p-4 border uppercase">{w._id.substring(0, 8)}</td>
                                            <td className="p-4 border font-bold text-gray-700 capitalize">{w.name}</td>
                                            <td className="p-4 border font-bold text-green-600">Satisfied {w.count} task</td>
                                        </tr>
                                    ))
                                ) : (
