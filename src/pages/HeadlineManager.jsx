...
const HeadlineManager = ({ type, title }) => {
    ...
    const fetchHeadlines = async () => {
        try {
            setLoading(true);
            const res = await api.get(`${API_ENDPOINTS.HEADLINES}?type=${type}`);
        ...
        <AdminLayout>
            <div className="p-4 md:p-6 bg-[#F4F6F9] min-h-screen">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-xl font-normal text-gray-800">Headline</h1>
                                                ...
                                                headlines.map((h, idx) => (
                                                    <tr key={h._id} className="hover:bg-gray-50 border-b">
                                                        ...
                                                        <td className="p-3 border text-center">
                                                            <button onClick={() => handleDelete(h._id)} className="bg-[#dc3545] text-white p-1.5 rounded"><FaTrash size={12} /></button>
                                                        </td>
                                                    </tr>
                                                ))
                                            ) : (
                        ...
                        <div className="bg-white rounded shadow-md overflow-hidden">
                            ...
                            <form onSubmit={handleSubmit} className="p-4 space-y-4">
                                <div>
                                    ...
                                    <textarea placeholder="Headline Title" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="w-full border rounded p-2 text-sm h-32" required></textarea>
                                </div>
                                <div>
                                    <label className="block text-xs font-bold mb-1">Link</label>
                                    <input type="text" placeholder="Link" value={formData.link} onChange={e => setFormData({ ...formData, link: e.target.value })} className="w-full border rounded p-2 text-sm" />
                                </div>
