...
const WithdrawMethodPage = () => {
    ...
    const fetchMethods = async () => {
        try {
            setLoading(true);
            const res = await api.get(API_ENDPOINTS.WITHDRAW_METHODS);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await api.post(API_ENDPOINTS.WITHDRAW_METHODS, { ...formData, icon: iconUrl });
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete this method?')) {
            try {
                await api.delete(`${API_ENDPOINTS.WITHDRAW_METHODS}/${id}`);
                fetchMethods();
            } catch (err) {
                console.error(err);
            }
        ...
        <AdminLayout>
            <div className="p-4 md:p-6 bg-[#F4F6F9] min-h-screen">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-xl font-normal text-gray-800">Withdraw Method</h1>
                    <div className="text-xs text-blue-500">Home / All Withdraw Method</div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div className="lg:col-span-8">
                        <div className="bg-white rounded shadow-md overflow-hidden">
                            <div className="bg-[#28a745] text-white p-3 font-bold text-sm">All Withdraw Method</div>
                            <div className="p-4 text-[11px] text-gray-600">
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full border-collapse">
                                        ...
                                        <tbody>
                                            {loading ? (
                                                <tr><td colSpan="5" className="p-10 text-center font-bold">Loading...</td></tr>
                                            ) : methods?.length > 0 ? (
                                                methods.map((m, idx) => (
                                                    <tr key={m._id} className="hover:bg-gray-50 uppercase border-b">
                    ...
                    <div className="lg:col-span-4">
                        <div className="bg-white rounded shadow-md overflow-hidden">
                            <div className="bg-[#007bff] text-white p-3 font-bold text-sm">New Method</div>
                            <form onSubmit={handleSubmit} className="p-4 space-y-4">
                                <div>
                                    <label className="block text-xs font-bold mb-1">Name</label>
                                    <input type="text" placeholder="Method Name" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} className="w-full border rounded p-2 text-sm" required />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold mb-1">Icon</label>
                                    <input type="file" onChange={handleFileChange} className="text-[10px]" />
                                </div>
                                <button type="submit" disabled={submitting} className="bg-[#007bff] text-white px-4 py-1.5 rounded text-xs font-bold shadow-md w-full">
...
export default WithdrawMethodPage;
