...
const TopJobPosterPage = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const [posterRes, headlineRes] = await Promise.all([
                api.get(`${API_ENDPOINTS.TOP_REPORTS}/top-job-posters`),
                api.get(`${API_ENDPOINTS.HEADLINES}?type=top-job-poster`)
            ]);
            setPosters(posterRes.data?.data || []);
    ...
    const handleHeadlineSubmit = async (e) => {
        ...
        try {
            setSubmitting(true);
            await api.post(API_ENDPOINTS.HEADLINES, { ...formData, type: 'top-job-poster' });
        ...
        <AdminLayout>
            <div className="p-4 md:p-6 bg-[#F4F6F9] min-h-screen">
                <div className="flex justify-between items-center mb-6">
                    <h1 className="text-xl font-normal text-gray-800">Top 15 Job Poster</h1>
                        ...
                        <table className="w-full border-collapse">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200 text-left text-[11px] font-bold text-gray-800 uppercase">
                                    ...
                                    <th className="p-4 border">Post</th>
                                </tr>
                            </thead>
                            <tbody className="text-[11px] text-gray-600">
                                {loading ? (
                                    ...
                                    posters.map((p, idx) => (
                                        <tr key={p._id} className="hover:bg-gray-50 border-b font-medium">
                                            <td className="p-4 border">{idx + 1}</td>
                                            <td className="p-4 border uppercase">{p._id.substring(0, 8)}</td>
                                            <td className="p-4 border font-bold text-gray-700 capitalize">{p.name}</td>
                                            <td className="p-4 border font-bold text-blue-600">{p.count} Job Posted</td>
                                        </tr>
                                    ))
                                ) : (
...
export default TopJobPosterPage;
